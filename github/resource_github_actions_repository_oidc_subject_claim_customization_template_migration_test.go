...
func Test_resourceGithubActionsRepositoryOIDCSubjectClaimCustomizationTemplateInstanceStateUpgradeV0(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range []struct {
	...
	}{
		{
			...
			rawState: map[string]any{
				...
				"use_default":        false,
				"include_claim_keys": []any{"repo", "context"},
			},
			want: map[string]any{
				"id":                 "test-repo",
				"repository":         "test-repo",
		...
		{
			testName:   "fails_if_repository_missing",
			statusCode: 404,
			body:       nil,
			rawState: map[string]any{
				"id": "test-repo",
		...
		t.Run(tt.testName, func(t *testing.T) {
			...
			got, err := resourceGithubActionsRepositoryOIDCSubjectClaimCustomizationTemplateInstanceStateUpgradeV0(t.Context(), tt.rawState, meta)
			if err != nil {
				if tt.wantErr == nil {
